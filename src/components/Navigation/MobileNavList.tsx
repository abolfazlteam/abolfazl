...
const MobileNavList: React.FC<INavListProps> = ({
  ...
  return createPortal(
    <ul
      // @ts-ignore
      ref={ref}
      className={`absolute left-4 top-[140px] z-40 flex w-[95%] flex-col gap-y-8 rounded-10 bg-bgColor pb-6 pl-6 pr-4 pt-4 transition-all duration-300 ease-linear sm:visible ${isMenuOpen ? "visible z-40 gap-y-8 opacity-100" : "invisible -z-30 scale-0 opacity-0"}`}
    >
